...
import com.tianxiabuyi.txutils_ui.datepicker.entities.DPInfo;
...
public class MonthView extends View {
    ...
    private boolean isNewEvent, isFestivalDisplay = true, isHolidayDisplay = true,
     ...
     * 切换至指定的年月
     */
    public void toSpecificMonth(int month, int year) {
        indexMonth = month;
        centerYear = year;
//        indexMonth++;
//        centerMonth = (centerMonth + 1) % 13;
...
//                    smoothScrollTo(width * indexMonth, totalMoveY);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 宽高比
        setMeasuredDimension(measureWidth, (int) (measureWidth * whRatio));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 画背景色
//        mPaint.setColor(Color.parseColor("#8ACEFF"));
//        canvas.drawRect(new Rect(0, 0, canvas.getWidth(), canvas.getHeight()), mPaint);

        drawBG(canvas, width * indexMonth, (indexYear - 1) * height, topYear, topMonth);
        drawBG(canvas, width * (indexMonth - 1), height * indexYear, leftYear, leftMonth);
        drawBG(canvas, width * indexMonth, indexYear * height, centerYear, centerMonth);
        drawBG(canvas, width * (indexMonth + 1), height * indexYear, rightYear, rightMonth);
        drawBG(canvas, width * indexMonth, (indexYear + 1) * height, bottomYear, bottomMonth);
        drawBGCircle(canvas);
    ...
    private void draw(Canvas canvas, int x, int y, int year, int month) {
        //Log.e("MonthView", "Year:Month " + year + " " + month);
        canvas.save();
    ...
    private void drawBG(Canvas canvas, int x, int y, int year, int month) {
        // Log.e("MonthView", "Year:Month " + year + " " + month);
        canvas.save();
        canvas.translate(x, y);
        DPInfo[][] info = mCManager.obtainDPInfo(year, month);
        ...
        for (int i = 0; i < result.length; i++) {
            for (int j = 0; j < result[i].length; j++) {
                DPInfo info1 = info[i][j];
                Rect rect = tmp[i][j].getBounds();
                if (null != mDPDecor && info1.isDecorBG) {
                    mDPDecor.drawDecorBG(canvas, rect, mPaint,
                            centerYear + "-" + centerMonth + "-" + info1.strG);
                    mPaint.setColor(Color.parseColor("#cce2f6"));
    ...
    private void draw(Canvas canvas, Rect rect, DPInfo info) {
        ...
        drawGregorian(canvas, rect, info.isDecorBG, info.strG, info.isToday, info.isSelected);
//        if (isFestivalDisplay) drawFestival(canvas, rect, info.strF, info.isToday, info.isFestival);
//        drawDecor(canvas, rect, info);
    ...
    private void drawBG(Canvas canvas, Rect rect, DPInfo info) {
        ...
        if (info.isToday && isTodayDisplay && cirApr.size() == 0) {
    ...
    private void drawGregorian(Canvas canvas, Rect rect, boolean isDecorBG, String str, boolean isToday, boolean isSelect) {
        mPaint.setTextSize(sizeTextGregorian);
        if (isDecorBG) {
            if (isSelect) {
                mPaint.setColor(mTManager.colorTitle());
            } else {
                mPaint.setColor(Color.parseColor("#238ceb"));
            }
        } else if (isToday && cirApr.size() == 0) {
            mPaint.setColor(mTManager.colorTitle());
        } else if (isSelect) {
            mPaint.setColor(mTManager.colorTitle());
        } else if (!isSelect && isToday) {
            mPaint.setColor(Color.parseColor("#74c200"));
        } else {
...
//            y = rect.centerY() + Math.abs(mPaint.ascent()) - (mPaint.descent() - mPaint.ascent()) / 2F;
        canvas.drawText(str, rect.centerX(), rect.centerY() + mPaint.getTextSize() / 2, mPaint);
    }

    private void drawFestival(Canvas canvas, Rect rect, String str, boolean isToday, boolean isSelected) {
        mPaint.setTextSize(sizeTextFestival);
     ...
     */
    String getShowMonth() {
        return centerYear + "-" + (centerMonth < 10 ? "0" + centerMonth : centerMonth);
    }
    ...
    private void defineRegion(int x, int y) {
        Log.e("aaaaaaaaYEAR", SELECTED_YEAR + "");
        Log.e("aaaaaaaaMONTH", SELECTED_MONTH + "");
        DPInfo[][] info = mCManager.obtainDPInfo(SELECTED_YEAR, SELECTED_MONTH);
        info[SELECTED_ROW][SELECTED_COL].isSelected = false;//把之前标记为选中的状态重置
        Region[][] tmp;
        if (TextUtils.isEmpty(info[4][0].strG)) {
            tmp = MONTH_REGIONS_4;
        } else if (TextUtils.isEmpty(info[5][0].strG)) {
            ...
            for (int j = 0; j < tmp[i].length; j++) {
                ...
                if (region.contains(x, y)) {
                    ...
                    if (mDPMode == DPMode.SINGLE) {
                        cirApr.clear();
                        regions.add(region);
                        final String date = centerYear + "-" + format(centerMonth) + "-" +
                                format(Integer.parseInt(mCManager.obtainDPInfo(centerYear, centerMonth)[i][j].strG));
                        mCManager.obtainDPInfo(centerYear, centerMonth)[i][j].isSelected = true;
                        SELECTED_ROW = i;
                        SELECTED_COL = j;
                        ...
                        SELECTED_YEAR = centerYear;
                        SELECTED_MONTH = centerMonth;
                        DPInfo[][] info1 = mCManager.obtainDPInfo(centerYear, centerMonth);
                        DPInfo infos = info1[SELECTED_ROW][SELECTED_COL];
                        infos.isSelected = true;
                        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
                            invalidate();
                            if (null != onDatePickedListener) {
                                onDatePickedListener.onDatePicked(date);
    ...
    interface OnDateChangeListener {
        ...
        void onDateChange(int month, int year);
    }
    ...
    /**
     * 把时间选择器，日期选择器中选择的结果格式化（比如8，转成08）
     *
     * @param value
     * @return
     */
