...
public class Hanzi2PinyinUtils {
    private static final String TAG = "Hanzi2PinyinUtils";
    ...
    private static Hanzi2PinyinUtils sInstance;
    ...
    protected Hanzi2PinyinUtils(boolean hasChinaCollator) {
        mHasChinaCollator = hasChinaCollator;
    }

    public static Hanzi2PinyinUtils getInstance() {
        synchronized (Hanzi2PinyinUtils.class) {
            if (sInstance != null) {
                return sInstance;
            }
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINESE)) {
                    ...
                    sInstance = new Hanzi2PinyinUtils(true);
                    return sInstance;
                }
            }
            Log.w(TAG, "There is no Chinese collator, Hanzi2PinyinUtils is disabled");
            sInstance = new Hanzi2PinyinUtils(false);
            return sInstance;
        }
    }
     ...
     * space will be put into a TokenResult, One Hanzi character which has pinyin will be treated as a
     * TokenResult. If these is no China collator, the empty token array is returned.
     */
